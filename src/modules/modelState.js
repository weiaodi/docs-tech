...
class ModelState {
    ...
    applyCommand(command, { editSource = EDIT_SOURCE_CONFIG.UNDO }) {
        console.log("🚀 ~ ModelState ~  ~ editSource:", editSource)
        console.log("🚀 ~ ModelState ~ applyCommand ~ command:", command)
    ...
    updateModel(command, { editSource = EDIT_SOURCE_CONFIG.USER_EDIT } = {}) {
    ...
    getCurrentBlock() {
        // this.updateBlocks();
        return (
            this.blocks_[this.cursorInfo_.locationY] || {
                html: "",
            }
        )
    }
    updateCursor() {
        ...
        this.cursorStyle_ = {
            left:
                PAGE_CONFIG.PADDING_LEFT +
                sizeUtil.getHtmlSize(block.html.substr(0, this.cursorInfo_.locationX)).width +
                "px",
